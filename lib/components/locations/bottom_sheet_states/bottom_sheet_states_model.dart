...
import 'bottom_sheet_states_widget.dart' show BottomSheetStatesWidget;
import 'dart:async';
...
class BottomSheetStatesModel extends FlutterFlowModel<BottomSheetStatesWidget> {
  ...
  // Stores action output result for [Custom Action - searchStatesInterceptorRequest] action in TextToSearch widget.
  String? statesSearchFilter;
  ...
  PagingController<ApiPagingParams, dynamic> _createListViewController(
    ...
    return controller..addPageRequestListener(listViewStatesPage);
  }

  void listViewStatesPage(ApiPagingParams nextPageMarker) =>
      listViewApiCall!(nextPageMarker).then((listViewStatesResponse) {
        final pageItems = (getJsonField(
                  listViewStatesResponse.jsonBody,
                  r'''$.data''',
                ) ??
                [])
            .toList() as List;
        ...
        listViewPagingController?.appendPage(
          ...
          (pageItems.length > 0)
              ? ApiPagingParams(
                  ...
                  lastResponse: listViewStatesResponse,
                )
              : null,
        );
      });

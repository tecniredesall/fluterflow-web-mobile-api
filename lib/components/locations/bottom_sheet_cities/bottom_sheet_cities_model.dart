...
import 'bottom_sheet_cities_widget.dart' show BottomSheetCitiesWidget;
import 'dart:async';
...
class BottomSheetCitiesModel extends FlutterFlowModel<BottomSheetCitiesWidget> {
  ...
  // Stores action output result for [Custom Action - searchCitiesInterceptorRequest] action in TextToSearch widget.
  String? citiesSearchFilter;
  ...
  PagingController<ApiPagingParams, dynamic> _createListViewController(
    ...
    return controller..addPageRequestListener(listViewCitiesPage);
  }

  void listViewCitiesPage(ApiPagingParams nextPageMarker) =>
      listViewApiCall!(nextPageMarker).then((listViewCitiesResponse) {
        final pageItems = (getJsonField(
                  listViewCitiesResponse.jsonBody,
                  r'''$.data''',
                ) ??
                [])
            .toList() as List;
        ...
        listViewPagingController?.appendPage(
          ...
          (pageItems.length > 0)
              ? ApiPagingParams(
                  ...
                  lastResponse: listViewCitiesResponse,
                )
              : null,
        );
      });
